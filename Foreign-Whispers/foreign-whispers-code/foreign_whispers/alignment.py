...
def global_align_dp(
    ...
    """Lookahead alignment that improves on greedy gap shifting.

    Greedy uses a silence gap whenever a segment qualifies for GAP_SHIFT.
    This version looks ahead a few segments and avoids spending silence on
    small overflows when a larger overflow is coming soon.
    """
    ...
    for i, m in enumerate(metrics):
        ...
        future_max_overflow = max(overflows[i + 1 : i + 4], default=0.0)

        if action == AlignAction.GAP_SHIFT:
            if m.overflow_s >= 0.25 and m.overflow_s >= future_max_overflow * 0.75:
